...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('coc_user', JSON.stringify(data.user))
      router.push('/')
      router.refresh()
    } catch (err: unknown) {
  ...
  return (
    <div style={{ maxWidth: 420, margin: '80px auto', padding: '0 24px' }}>
      <div style={{ marginBottom: 32, textAlign: 'center' }}>
        ...
        <h1 style={{ fontWeight: 700, fontSize: '1.5rem', marginBottom: 8 }}>Sign in</h1>
        <p style={{ color: 'var(--muted)', fontSize: '0.9rem' }}>
          No account yet? <Link href="/register" style={{ color: 'var(--foreground)' }}>Create one</Link>
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ textAlign: 'center', color: 'var(--muted)', fontSize: '0.8rem', margin: '4px 0' }}>
          or
        </div>

        <a href={`${process.env.NEXT_PUBLIC_API_URL}/api/v1/auth/google`} style={oauthButtonStyle}>
        ...
        <a href={`${process.env.NEXT_PUBLIC_API_URL}/api/v1/auth/github`} style={{ ...oauthButtonStyle, marginTop: 0 }}>
